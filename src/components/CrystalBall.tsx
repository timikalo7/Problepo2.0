...
const CrystalBall: React.FC<CrystalBallProps> = ({ isAnalyzing }) => {
  return (
    <motion.div
    ...
    >
      <div className="w-64 h-64 md:w-80 md:h-80 rounded-full relative overflow-hidden">
        ...
        <div className="absolute inset-2 rounded-full bg-gradient-to-br from-gray-900 to-gray-800 overflow-hidden backdrop-blur-sm border border-gray-700/50">
          ...
          <motion.div
            animate={{
              opacity: isAnalyzing ? [0.4, 0.7, 0.4] : 0.4,
      ...
      {isAnalyzing && (
        ...
        >
          <p className="text-lg font-medium">Consulting the financial spirits...</p>
          <p className="text-sm text-gray-400">The crystal ball is analyzing market sentiments</p>
        </motion.div>
      )}
    </motion.div>
  );
